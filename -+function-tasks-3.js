...
console.group("4. Sukurkite funkciją, kuri atspausdina pirmojo parametro paskutinę raidę");
{
  ...
  console.log(printLastLetter("azbukamorze"))
   console.log('---');
...
console.group("5. Sukurkite funkciją, kuri atspausdina pirmojo parametro viduriniają raidę");
{
  ...
  console.log( {
   res: printMiddleLetter('Labas'),
   asd: printMiddleLetter('Ate'),
  edma: printMiddleLetter('Brieda')
  })
   console.log('---');
}
console.groupEnd();
...
console.group("9. Sukurkite funkciją, kuri grąžina <true>, jeigu žodyje yra 'a' raidė, priešingu atveju <false>");
{
  ...
  console.log(containsLetterA('Edma'))
   console.log('---');
   console.log({
...
console.group("10. Sukurkite funkciją, kuri grąžina <true>, jeigu žodyje yra antrojo parametru perduoda raidė, priešingu atveju false");
{
...
console.group("12. Sukurkite funkciją, kuri grąžina balsių kiekį žodyje");
{
  ...
  function vowels(ch) {
    return ch == "a"|| ch == "o"|| ch =="ą"|| ch=="ė" || ch == "y" || ch == "i" || ch == "u"
  }
...
console.group("14. Sukurkite funkciją, kuri grąžina <true>, jeigu NĖRA skaičių");
{
  ...
  // Pasikartoti itteracinius metodus [!] - forEach
...
console.group("18. Sukurkite funkciją, kuri grąžina pirmos (nuo kairės pusės) vartotojo perduotos raidės vietą žodyje");
{
  function lastIndexOfLetter(str, searchLetter) {
   ...
   console.log({
     ...
     'kempės, k': lastIndexOfLetter('empėsk', 'k'),
...
console.group("19. Sukurkite funkciją, kuri grąžina indeksų masyvą su visais 'a' raidės pasikartojimais žodyje");
{
  ...
  // by using str[i] we wil get each element element
   console.log('---');
 console.log({
...
console.group("23. Sukurkite funkciją, kuri ištrintų visus 'a' raidės pasikartojimus žodyje");
{
  function removeAllOccurencesOfLetterA(str) {
   ...
   console.log({
     ...
     'kempiniukas': removeAllOccurencesOfLetterA('kempainiukas'),
...
console.group("25. Sukurkite funkciją, kuri pirmu parametru priimtų žodį, o antruoju - masyvą su raidėmis."); //[!] persiziuret
{

  function filterLetters(str, lettersToRemove) {
    const searchFor = new RegExp('[' + lettersToRemove.join('') + ']+', 'g');
    return str.replace(searchFor, '');
  }
  console.log('---');
  const str = filterLetters('Brangakmienio paveikslas', ['B', 'p']);
  console.log(str);
  console.log('---');
...
console.group("28. Sukurkite funkciją, kuri atspausdina žodį atvirkščiai 'abc' -> 'cba'");
{
  function reverseString(s){
   ...
   console.log('---');
   console.log({
    'viens du trys': reverseString('viens du trys'),
    'as tave myliu': reverseString('as tave myliu'),
    'Bairis seniuk': reverseString('Bairis seniuk'),
   })
   console.log('---');
}
console.groupEnd();
...
console.group("30. Sukurkite funkciją, kuri taiso pastraipos klaidas"); //[!] reikia pabaigt
{

  function capitalizeFirstWord(str) {
    return str[0].charAt(0).toUpperCase() + str.slice(1)
    // change at index 0 first leeter to upper case then add sliced word to that letter
  }

  function splitIntoSentences(paragraph) {
    // const splitStr = paragraph.replace(/\s+/g, ' ').replace(/\s*,\s*/g, ", ").trim()
  ...
  function reduceEmptySpaces(str) {
    const splitStr = str.replace(/\s+/g, '')
    const splitted = splitStr.split(' ')
    return splitted
  ...
  function fixParagraph(paragraph) {
   ...
   console.log(capitalizeFirstWord('sveikas'))
...
console.group("32. Sukurkite funkciją, kuri pirmu parametru priimą <string>'ą, o antruoju skiriklį"); // [!] reikia pabaigt
{
  function explode(str, separator) {
    let result = [];
    const splitted = str.replace(/[^\w\s]/gi, '  ').split(' ')
    for (let i = 0; i < splitted.length; i++) {
        result +=  splitted[i] + separator
    }
    console.log(result)
    return result.split(' ')
  }
...
// {} curly braces to destruct object, also we create an object using curly
// All we have to do is to provide the variable names that exactly match the property names
// that we want to retrieve from the object
// in an object the order of elements does not matter, we don't need manually
// skip elements like we did in an array with the hole


//name, categories and opening openingHourse

// based on restaurant object which is defined from right side
